...
describe('GrabCraft blueprint import', () => {
  ...
  it('treats the prepared ground as support but skips blocks floating above it', async () => {
    const page = `
      <h1>Floating Glass</h1><h3>Author:&nbsp;Builder<br />Block count:&nbsp;1<br /></h3>
      <script src="/js/RenderObject/myRenderObject_4.js"></script>
      <script>var dimX = 1; var dimY = 3; var dimZ = 1; var totalPositions = 3;</script>
    `;
    const render = renderAssignment([{ x: 1, y: 3, z: 1, name: 'Glass', materialId: '20' }]);
    const blueprint = await importGrabcraftBlueprint(
